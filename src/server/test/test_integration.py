...
def model(config_path):
    ...
    model = model_class(model_name, model_config, data_config, train_config, eval_config)
    # str(model_class)
    return model
...
def test_train_eval_run(data_train, data_eval, model):
    ...
    if model.model_name == "CustomCellpose": masks = [mask[0] for mask in masks] 
    model.train(images, masks)
   
    imgs_test, masks_test = data_eval
    if model.model_name == "CustomCellpose": masks = [mask[0] for mask in masks_test] 
