...
class CustomCellpose(models.CellposeModel, Model):
    ...
    def eval_all_outputs(self, img: np.ndarray) -> tuple:
    ...
    def compute_masks_flows(
        ...
        for idx, img in enumerate(imgs):
            ...
            pred_flows.append(
                np.stack([flows[1][0], flows[1][1], flows[2]])
            )  # stack cell probability map, horizontal and vertical flow
            true_lbl.append(
                np.stack([true_bin_masks[idx], true_flows[idx][2], true_flows[idx][3]])
            )
    ...
    def masks_to_outlines(self, mask: np.ndarray) -> np.ndarray:
