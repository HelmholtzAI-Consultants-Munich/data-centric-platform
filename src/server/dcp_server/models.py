...
class CellposePatchCNN(nn.Module):
    ...
    def train(self, imgs, masks):
        ...
        patches, labels = create_patch_dataset(
            imgs,
            masks_classes,
            masks_instances,
            noise_intensity=self.train_config["classifier"]["train_data"][
                "noise_intensity"
            ],
    ...
    def eval(self, img):
        ...
        with torch.no_grad():
            ...
            patches, instance_labels, _ = get_centered_patches(
                img, instance_mask, max_patch_size, noise_intensity=noise_intensity
            )
            # loop over patches and create classification mask
            for idx, patch in enumerate(patches):
                patch_class = self.classifier.eval(
                    patch
